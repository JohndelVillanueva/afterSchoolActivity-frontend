...
const StudentsPage: React.FC = () => {
  ...
  const MobileTopBar = (
    <div className="md:hidden flex items-center bg-white border-b border-gray-200 px-4 py-3 sticky top-0 z-30 shadow-sm">
      ...
      <span className={`text-lg font-semibold text-${TEXT_COLOR}`}>Students</span>
    </div>
  );
  ...
  const handleRfidCheck = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const existingUser = result.data.find((user: any) => 
          user.rfid && user.rfid.trim() === form.rfid.trim()
        );
    ...
    <div className={`min-h-screen flex bg-${BG_COLOR}`}>
      ...
      <div className={`flex-1 min-h-screen transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'md:ml-16' : 'md:ml-64'} flex flex-col`}>
        ...
        <header className="border-b border-gray-200 py-6 px-4 md:py-8 md:px-8 bg-white flex flex-col md:flex-row md:items-center md:justify-between gap-4 sticky top-0 z-20 shadow-sm">
          <div>
            <h1 className={`text-2xl md:text-3xl font-bold text-${TEXT_COLOR}`}>Students Roster 🧑‍🎓</h1>
            <p className="text-gray-500 text-sm mt-1">Manage your afterschool student roster and enrollment status.</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              value={statusFilter}
              onChange={e => setStatusFilter(e.target.value as 'all' | 'enrolled' | 'not_enrolled')}
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          {loading ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl shadow-md border border-dashed border-gray-200">
              <h2 className={`text-xl font-semibold text-${TEXT_COLOR} mb-2`}>No students found</h2>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className={`bg-${PRIMARY_LIGHT} sticky top-0`}>
                    <tr>
                      <th className={`px-6 py-3 text-left text-xs font-semibold text-${TEXT_COLOR} uppercase tracking-wider`}>Student</th>
                      <th className={`px-6 py-3 text-left text-xs font-semibold text-${TEXT_COLOR} uppercase tracking-wider`}>RFID</th>
                      <th className={`px-6 py-3 text-left text-xs font-semibold text-${TEXT_COLOR} uppercase tracking-wider`}>Email</th>
                      <th className={`px-6 py-3 text-left text-xs font-semibold text-${TEXT_COLOR} uppercase tracking-wider`}>Sessions (A/P)</th>
                      {/* <th className={`px-6 py-3 text-left text-xs font-semibold text-${TEXT_COLOR} uppercase tracking-wider`}>Status</th> */}
                      <th className={`px-6 py-3 text-right text-xs font-semibold text-${TEXT_COLOR} uppercase tracking-wider`}>Actions</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-sm font-semibold text-${PRIMARY_COLOR}`}>
                              {`${student.fname?.[0] || ''}${student.lname?.[0] || ''}`.toUpperCase()}
                            </div>
                            <div>
                              <div className={`text-sm font-semibold text-${TEXT_COLOR}`}>{student.fname} {student.lname}</div>
                              {/* <div className="text-xs text-gray-500">ID: {student.id}</div> */}
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-mono">{student.rfid || '—'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600">
                          {student.email ? (
                            <a href={`mailto:${student.email}`} className="hover:underline text-blue-500">{student.email}</a>
                          ) : (
                            <span className="text-gray-400">No email</span>
                          )}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <span className="text-yellow-600"> {student.sessionsAttended ?? '0'}</span> /  
                            <span> </span>
                            <span className="text-green-600">{student.sessionsPurchased ?? '0'}</span>
                        </td>
                        {/* <td className="px-6 py-4 whitespace-nowrap">
        ...
        {formStep === 'rfid' ? (
          ...
          <form onSubmit={handleCreateStudent} className="space-y-6">
              ...
              <div className="p-5 border-2 border-gray-900 bg-gray-50">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">{existingStudent.email || 'No email'}</p>
                    <p className="text-xs text-gray-500 font-mono mt-2">RFID: {existingStudent.rfid}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-900">Enrollment Status</label>
                <select
                  name="status"
                  value={form.status}
                  onChange={handleInputChange}
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-900">Sessions Purchased</label>
              <input
                type="number"
                name="sessions"
                min="0"
